...
import { apiResultFormat, order, pageSelection } from 'src/app/shared/models/models';
...
export class OrderComponent {
  ...
  constructor(
    ...
    this.pagination.tablePageSize.subscribe((res: tablePageSize) => {
      if (this.router.url == this.routes.order) {
  ...
  private getTableData(pageOption: pageSelection): void {
    this.data.getOrder().subscribe((apiRes: apiResultFormat) => {
      this.tableData = [];
      this.serialNumberArray = [];
      this.totalData = apiRes.totalData;
      apiRes.data.map((res: order, index: number) => {
      ...
      this.dataSource = new MatTableDataSource<order>(this.tableData);
