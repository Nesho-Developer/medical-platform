...
import { expired, apiResultFormat, pageSelection } from 'src/app/shared/models/models';
...
export class ExpiredComponent {
  ...
  public totalData = 0;
  showFilter = false;
  dataSource!: MatTableDataSource<expired>;
  ...
  private getTableData(pageOption: pageSelection): void {
    this.data.getExpired().subscribe((apiRes: apiResultFormat) => {
      this.tableData = [];
      this.serialNumberArray = [];
      this.totalData = apiRes.totalData;
      apiRes.data.map((res: expired, index: number) => {
      ...
      this.dataSource = new MatTableDataSource<expired>(this.tableData);
