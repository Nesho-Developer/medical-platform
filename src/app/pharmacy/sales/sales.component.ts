...
import { sales, apiResultFormat, pageSelection } from 'src/app/shared/models/models';
...
export class SalesComponent {
  ...
  constructor(
    ...
    this.pagination.tablePageSize.subscribe((res: tablePageSize) => {
      if (this.router.url == this.routes.sales) {
  ...
  private getTableData(pageOption: pageSelection): void {
    this.data.getSales().subscribe((apiRes: apiResultFormat) => {
      this.tableData = [];
      this.serialNumberArray = [];
      this.totalData = apiRes.totalData;
      apiRes.data.map((res: sales, index: number) => {
      ...
      this.dataSource = new MatTableDataSource<sales>(this.tableData);
