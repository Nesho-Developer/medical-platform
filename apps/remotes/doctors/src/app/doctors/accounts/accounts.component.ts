...
import { accounts, apiResultFormat, pageSelection } from '@nts/shared';
...
export class AccountsComponent {
  ...
  private getTableData(pageOption: pageSelection): void {
    this.data.getAccounts().subscribe((apiRes: apiResultFormat) => {
      this.tableData = [];
      this.serialNumberArray = [];
      this.totalData = apiRes.totalData;
      apiRes.data.map((res: accounts, index: number) => {
      ...
      this.dataSource = new MatTableDataSource<accounts>(this.tableData);
